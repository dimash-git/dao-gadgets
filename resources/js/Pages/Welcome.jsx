...
export default function Welcome({ auth, laravelVersion, phpVersion }) {
    ...
    // props которые переданы через руты в web.php
    // auth это глобольный props который передается через middleware Laravel на все приложение
    return (
        <>
            <Head title="Welcome" />
                    ...
                    {auth.user ? (
                        <div className="flex gap-2 gap-x-2">
                            {auth.user?.roles[0]?.name === "admin" ? (
                                <Link
                                    href={route("admin")}
                                    // route - функция которая генерирует ссылку которая соотносится с рутом бэка, идет вместе с установкой breeze
